"""
Garment Sparse BPT Converter - Final fixed version with improved error handling
...
def optimize_garment_sparse_quads_builtin():
    ...
    try:
        ...
        # 使用弧度值而不是mathutils.radians
        try:
            bpy.ops.mesh.limited_dissolve(angle_limit=0.5236)  # 30 degrees in radians
        except Exception as e:
        ...
        try:
            bpy.ops.mesh.tris_convert_to_quads(
                face_threshold=0.5236,  # 30 degrees in radians
                shape_threshold=0.5236  # 30 degrees in radians
            )
        except Exception as e:
        ...
        # 优化网格布局
        try:
            bpy.ops.mesh.vert_connect_nonplanar(angle_limit=0.5236)  # 30 degrees in radians
        except Exception as e:
            print(f"Warning: vert_connect_nonplanar failed: {str(e)}")

        # 再次进行limited dissolve以进一步稀疏化
        try:
            bpy.ops.mesh.limited_dissolve(angle_limit=0.1745)  # 10 degrees in radians
        except Exception as e:
...
def main():
    ...
    output_directory = "D:\\ClothesNetData_Sparse_Quads_Final"
