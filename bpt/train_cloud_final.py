...
class SupervisedDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.cache_dir:
            ...
            out["cond_embeds"] = emb.squeeze(0).float()
        else:
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch in pbar:
            ...
            loss = None  # init before if/else

            if "cond_embeds" in batch:
                cond_kw = {"cond_embeds": batch["cond_embeds"].to(device)}
            else:
                cond_kw = {"pc": batch["pc_normal"].to(device).float()}

            # autocast disabled for fp32
                try:
            ...
            if (global_step + 1) % args.grad_accum_steps == 0:
                # scaler.unscale_ disabled
                torch.nn.utils.clip_grad_norm_(trainable, args.grad_clip)
                optimizer.step()
                # scaler.update() disabled
                scheduler.step()
                optimizer.zero_grad()
        ...
        if avg_val_loss < best_val_loss:
            ...
            patience_counter += 1
            print(f"  val_loss stagnant ({patience_counter}/3)")
