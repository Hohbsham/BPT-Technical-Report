...
parser.add_argument('--mesh_type', choices=['triangle','quad','auto'], default='auto',
                    help='Output mesh face type')
    ...
    with torch.no_grad():
        for it, data in enumerate(dataloader):
            ...
            try:
                ...
                for i in range(len(codes)):
                    ...
                    vertices = BPT_deserialize(
                        ...
                        mesh_type = args.mesh_type if args.mesh_type != 'auto' else 'triangle',
                    )
            ...
            for i in range(args.batch_size):
                uid = data['uid'][i]
                vertices = coords[i]
                face_width = 4 if args.mesh_type == 'quad' else 3
                faces = torch.arange(1, len(vertices) + 1).view(-1, face_width)
